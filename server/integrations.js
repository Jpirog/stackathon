...
const getTwitterUpdates = async () => {
  ...
  const tweets = await twitterClient.v2.userTimeline(user.data.id, { 'tweet.fields': 'author_id,id,text,created_at', expansions:'author_id', 'user.fields':'name' })
...
const checkAppropriateness = async (Alert, textIn) => {
  ...
  try{
    ...
    // const data = await axios.post('https://api.sightengine.com/1.0/text/check.json', {
    //   lang: 'en',
    //   text: encodeURI(textIn),
    //   mode: 'standard',
